...
def load_model_and_preprocessor():
    ...
    def part_of_day(X, morning=4, noon=12, eve=16, night=20):
        columns = X.columns.to_list()
        X_temp = X.assign(**{col: pd.to_datetime(X[col]).dt.hour for col in columns})
    ...
    class RBFPercentileSimilarity(BaseEstimator, TransformerMixin):
        ...
        def transform(self, X):
            ...
            for col in self.variables:
                ...
                obj = pd.DataFrame(
                    data=rbf_kernel(X[[col]], Y=self.reference_values_[col], gamma=self.gamma),
                    columns=columns
                )
                objects.append(obj)
...
preprocessor, model, X_train = load_model_and_preprocessor()

st.title("Flight Price Prediction ✈️")
st.markdown("Predict Indian domestic flight prices based on your travel details.")
...
with col2:
    ...
    duration        = st.number_input("Duration (mins):", step=1, min_value=0)
...
if st.button("Predict Price"):
    ...
    st.success(f"✈️ Estimated Flight Price: ₹{pred:,.0f} INR")
